...
export function useWcagColors(): UseWcagColorsReturn {
  ...
  const getColorPair = useCallback(
    (category: string, variant: string, standard: WcagStandard = WcagStandard.AA) => {
  ...
  const getCategoryPairs = useCallback(
    (category: string, standard: WcagStandard = WcagStandard.AA) => {
  ...
  useMemo(() => {
    ...
    if (!validation.isValid) {
      ...
      if (alternatives.length > 0) {
        // Development info - accessible color alternatives available
        // console.info(`Accessible color alternatives available`);
      }
    }
