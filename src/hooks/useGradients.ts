...
export function useGradients(): UseGradientsReturn {
  ...
  const getGradientMemo = useCallback((
    category: GradientCategory,
    variant: GradientVariant
  ) => {
  ...
  const getGradientRecommendationsMemo = useCallback((context: {
    usage: string;
    accessibility?: {
      highContrast?: boolean;
      darkMode?: boolean;
    ...
    if (requirements.accessibility) {
      ...
      if (!accessibilityValidation.isValid) {
        console.warn(
          ...
          accessibilityValidation.issues.map(issue => `  - ${issue}`).join('\n') +
          '\n\nRecommendations:\n' +
          accessibilityValidation.recommendations.map(rec => `  - ${rec}`).join('\n')
        );
      }
    }
    ...
    if (requirements.performance) {
      ...
      if (performanceIssues.length > 0) {
        ...
        if (alternatives.length > 0) {
          console.info(
            `💡 Suggested performant alternatives:\n` +
            alternatives.map(alt => `  - ${alt.name}: ${alt.description}`).join('\n')
          );
        }
      }
    }
  }, [gradient, requirements, componentName, validateGradientAccessibility, getGradientRecommendations]);
}
